"""benchmark_automated_v2.py — Automated transcription benchmarking tool.
...
def benchmark(model_name, num_workers=1, compute_type="int8", 
    ...
    print(f"Workers: {num_workers}, Compute: {compute_type}")
    ...
    model = WhisperModel(model_name, device=device, compute_type=compute_type, num_workers=num_workers)
...
def main():
    ...
    if config_name == "baseline":
        ...
        results = benchmark("bofenghuang/whisper-large-v3-distil-fr-v0.2", beam_size=1, condition_on_previous_text=False)
        
    elif config_name == "workers":
        # Opt C: num_workers=4
        results = benchmark("small", num_workers=4, beam_size=1, condition_on_previous_text=False)
        
    elif config_name == "compute":
        # Opt D: int8_float16
        results = benchmark("small", compute_type="int8_float16", beam_size=1, condition_on_previous_text=False)
